...
function inferSocialGoal(mode, tone, polish, energy, selfFeel, platform) {
  ...
  if (text.includes('instagram') || text.includes('rednote') || text.includes('tiktok')) {
...
function inferReaderEffect(mode, tone, polish, energy, selfFeel) {
  ...
  if (mode === 'Soft intimate storyteller') return `make the reader feel quietly invited in, emotionally close, and ${calmExpressive}`;
  if (mode === 'Polished lifestyle sharer') return `make the reader feel tasteful admiration, aspiration, and ${softSharp}`;
  if (mode === 'Trend-aware curator') return `make the reader feel current, interested, and socially in-the-know`;
  if (mode === 'Smart playful explainer') return `make the reader feel entertained, smart, and pleasantly surprised`;
...
function inferDistinctivenessStrategy(mode, tone, polish, energy, selfFeel, vibes) {
  ...
  items.push('use at least one concrete, memorable detail rather than generic summary');
  items.push('give each draft a different social angle, not just different wording');

  if (num(tone) <= 35) {
    items.push('prefer implication, specificity, and emotional restraint over loud claims');
  } else if (num(tone) >= 67) {
  ...
  if (num(polish) >= 67) {
    ...
    items.push('retain a little messiness or lived-in texture so the post feels human');
  }

  if (num(energy) >= 67) {
    items.push('increase momentum, variation, and lift without sounding forced');
  ...
  if (mode === 'Polished lifestyle sharer') {
    items.push('build distinctiveness through tasteful curation and understated social desirability');
  }
  ...
  if (vibesText.includes('cozy') || vibesText.includes('dreamy')) {
    items.push('favor sensory warmth and atmospheric softness');
  }
  if (vibesText.includes('playful')) {
    items.push('favor charm, surprise, and light delight');
  }

  if (selfText.includes('soft')) items.push('translate softness into intimacy and elegance, not weakness or vagueness');
  if (selfText.includes('appealing')) items.push('make the post socially attractive without sounding try-hard');
  if (selfText.includes('distinct')) items.push('make the post feel recognizably specific to one person, not template-like');

  return items;
}

function inferRestraintRules(mode, tone, polish, energy, platform) {
  const items = [
    'avoid generic motivational language',
    'avoid obvious AI cadence and symmetrical phrasing',
    'avoid clichés, empty uplift, and vague universal statements',
...
function inferStructureStrategies(mode, platform, energy) {
  const base = [
    ...
    'Draft 2: appeal-first, with stronger hook and social attractiveness',
...
export function compileCharacterStrategy({
  ...
  return {
    ...
    reader_effect: inferReaderEffect(resolvedMode, tone, polish, energy, resolvedFeel),
    distinctiveness_strategy: inferDistinctivenessStrategy(
      resolvedMode, tone, polish, energy, resolvedFeel, resolvedVibes
    ),
    restraint_rules: inferRestraintRules(resolvedMode, tone, polish, energy, resolvedPlatform),
...
- Use specific detail, framing, rhythm, and restraint to create effect.
